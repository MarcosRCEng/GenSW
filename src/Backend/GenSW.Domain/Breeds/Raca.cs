...
public sealed class Raca
{
    ...
    private Raca(Guid especieId, string nome, DateTimeOffset nowUtc)
    {
        Id = Guid.NewGuid();
        EspecieId = especieId;
        Nome = nome;
    ...
    public static Raca Criar(Guid especieId, string nome, DateTimeOffset nowUtc)
    ...
    private static string NormalizeRequiredName(string value, string parameterName)
    {
        ...
        if (normalized.Length is < 1 or > NomeMaxLength)
        {
            throw new ArgumentException(
                "The breed name must have between 1 and 200 characters.",
                parameterName);
        }
